...
#else
static __inline__ void bolero_clk_rsc_fs_gen_request(struct device *dev,
						bool enable)
{
}
static __inline__ int bolero_clk_rsc_mgr_init(void)
{
	return 0;
}
static __inline__ void bolero_clk_rsc_mgr_exit(void)
{
}
static __inline__ int bolero_clk_rsc_request_clock(struct device *dev,
				int default_clk_id,
				int clk_id_req,
				bool enable)
{
...
static __inline__ int bolero_rsc_clk_reset(struct device *dev, int clk_id)
{
	return 0;
}
static __inline__ void bolero_clk_rsc_enable_all_clocks(struct device *dev,
						    bool enable)
{
	return;
}
