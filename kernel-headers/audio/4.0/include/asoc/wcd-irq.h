...
#else
static __inline__ int wcd_irq_init(struct wcd_irq_info *irq_info,
			       struct irq_domain **virq)
{
	return 0;
};
static __inline__ int wcd_irq_exit(struct wcd_irq_info *irq_info,
			       struct irq_domain *virq)
{
	return 0;
};
static __inline__ int wcd_request_irq(struct wcd_irq_info *irq_info,
				  int irq, const char *name,
				  irq_handler_t handler, void *data)
{
	return 0;
};
static __inline__ void wcd_free_irq(struct wcd_irq_info *irq_info, int irq, void *data);
{
};
static __inline__ void wcd_enable_irq(struct wcd_irq_info *irq_info, int irq);
{
};
static __inline__ void wcd_disable_irq(struct wcd_irq_info *irq_info, int irq);
{
};
#endif
#endif /* __WCD_IRQ_H_ */
