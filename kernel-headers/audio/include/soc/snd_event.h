...
void snd_event_mstr_add_client(struct snd_event_clients **snd_clients,
			    ...
			    void *data);
__inline__ bool is_snd_event_fwk_enabled(void)
{
	return 1;
}
#else
static __inline__ int snd_event_client_register(struct device *dev,
			      const struct snd_event_ops *snd_ev_ops,
			      void *data)
{
	return 0;
}
static __inline__ int snd_event_client_deregister(struct device *dev)
{
	return 0;
}
static __inline__ int snd_event_master_register(struct device *dev,
			      const struct snd_event_ops *ops,
			      struct snd_event_clients *clients,
			      void *data)
{
...
static __inline__ int snd_event_master_deregister(struct device *dev)
{
	return 0;
}
static __inline__ int snd_event_notify(struct device *dev, unsigned int state)
{
	return 0;
}

static __inline__ void snd_event_mstr_add_client(struct snd_event_clients **snd_clients,
			    int (*compare)(struct device *, void *),
			    void *data)
{
	return;
}
static __inline__ bool is_snd_event_fwk_enabled(void)
{
	return 0;
}
